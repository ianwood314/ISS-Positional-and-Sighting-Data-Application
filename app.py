...
def download_data(iss_pos_filename = 'ISS.OEM_J2K_EPH.xml', sightings_filename = 'XMLsightingData_citiesUSA01.xml'):
	'''
	...
	try:
		...
		logging.error(e)
		return filenotfound_error_str + 'The ISS positional data XML file was not\n\n'

	try:
		with open(sightings_filename, 'r') as f:
			data = xmltodict.parse(f.read())
		sightings_data = data['visible_passes']['visible_pass']

		logging.debug('Download data queried')
	except FileNotFoundError as e:
		logging.error(e)
		return filenotfound_error_str + 'The ISS sightings data XML file was not\n\n'
	return '\n-- Download Successful --\n\n'
...
@app.route('/epochs/<requested_epoch>', methods=['GET'])
def get_epoch(requested_epoch: str) -> dict:
	'''
	Takes in user input of an epoch and returns the first dictionary 
	whose EPOCH key value matches the user's unput

	Args:
		requested_epoch (str): the requested epoch by the user

	Returns:
		epoch (dict): a dictionary containing all the information about the epoch
		requested by the user
	'''
	try:
		for epoch in iss_pos_data:
			if epoch[epoch_key] == requested_epoch:
				logging.debug('Get a specific epoch queried')
